...
I'm Yuzi, a phD student in the cereal quality group of Ag & food. Without any programming experience before, the data school is sort of a whole new world for me, and it turned out to be very intereting and I'm keen to learn more.
...
The current dataset is a combination of my experimental results (hydrolysis) on 200 wheat starch and also some previous results from other people regarding to the structural properties. The hydrolysis assay was done in microplates, it's an enzymatic reaction over 30 hours with several sampling from time to time.
  ...
  geom_point(data = total_data, 
    ...
    geom_line(data = x,
          aes(x = Time,
              ...
              group = Well,
              color = status),
          size = 0.05) + # make the line thinner
  scale_color_manual(values = c("#e41a1c", "#4daf4a", "#377eb8")) +
  scale_y_continuous(limits = c(0,100), expand = c(0, 0)) + ## set the range of the y axis with no more expand
  scale_x_continuous(breaks = c(500, 1500),
                     limits = c(0, 1900), expand = c(0, 0)) +
...
- Try other mathematical equation to do the curve fitting, compare the goodness of fitting
...
It's an awesome learning experience. With a nice pace, the explanation in detail and lots of practical days, I start to see the beauty of R and really enjoy exploring it. I've already applied what I've learnt in data school to my daily work, and it's always exciting to discover more packages and codes that help us solve various problems. 
